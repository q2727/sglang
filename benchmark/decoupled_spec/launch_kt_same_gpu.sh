...
DRAFT_PAGE_SIZE="${DRAFT_PAGE_SIZE:-64}"
TARGET_MAX_TOKENS="${TARGET_MAX_TOKENS:-8192}"
DRAFT_MAX_TOKENS="${DRAFT_MAX_TOKENS:-16384}"
...
DRAFT_CHAIN_GRAPH="${DRAFT_CHAIN_GRAPH:-1}"
DRAFT_PRELAUNCH="${DRAFT_PRELAUNCH:-1}"
DRAFT_CHAIN_PRELAUNCH="${DRAFT_CHAIN_PRELAUNCH:-1}"
DRAFT_DEVICE_PACK="${DRAFT_DEVICE_PACK:-1}"
DRAFT_PER_CASE_FANOUT="${DRAFT_PER_CASE_FANOUT:-0}"
DRAFT_PER_CASE_FANOUT_BUDGETS="${DRAFT_PER_CASE_FANOUT_BUDGETS:-}"
DRAFT_BET_PREBUILD="${DRAFT_BET_PREBUILD:-0}"

if (( K < 1 || F < 1 )); then
  echo "K and F must both be positive." >&2
  exit 2
fi
if (( DRAFT_PAGE_SIZE < 1 )); then
  echo "DRAFT_PAGE_SIZE must be positive." >&2
  exit 2
fi
if (( DRAFT_PRELAUNCH == 1 && DRAFT_PAGE_SIZE == 1 )); then
  echo "DRAFT_PRELAUNCH=1 requires paged KV; set DRAFT_PAGE_SIZE>1 (default: 64)." >&2
  exit 2
fi
if (( TARGET_SM_PERCENT < 1 || TARGET_SM_PERCENT > 100 || DRAFT_SM_PERCENT < 1 || DRAFT_SM_PERCENT > 100 )); then
  echo "Each SM percentage must be within [1, 100]." >&2
  exit 2
fi

if (( DRAFT_PAGE_SIZE > 1 )); then
  DRAFT_PRIVATE_SPAN=$((DRAFT_PAGE_SIZE - 1 + (K + 1) + K + K))
  DRAFT_PAGES_PER_ROW=$(((DRAFT_PRIVATE_SPAN + DRAFT_PAGE_SIZE - 1) / DRAFT_PAGE_SIZE))
  DRAFT_MIN_ARENA_PAGES=$(((K + (K + 1) * F + 1) * DRAFT_PAGES_PER_ROW + 1))
  DRAFT_MIN_ARENA_TOKENS=$((DRAFT_MIN_ARENA_PAGES * DRAFT_PAGE_SIZE))
  if (( DRAFT_MAX_TOKENS <= DRAFT_MIN_ARENA_TOKENS )); then
    echo "DRAFT_MAX_TOKENS=${DRAFT_MAX_TOKENS} cannot hold the paged SSD arena (${DRAFT_MIN_ARENA_TOKENS} tokens) plus the live prefix." >&2
    exit 2
  fi
fi
...
setsid env \
  SGLANG_SKIP_SGL_KERNEL_VERSION_CHECK=1 \
  SGLANG_ENABLE_DECOUPLED_CHAIN_GRAPH="${DRAFT_CHAIN_GRAPH}" \
  SGLANG_DECOUPLED_ENUM_WAIT_MS="${SGLANG_DECOUPLED_ENUM_WAIT_MS:-200}" \
  SGLANG_ENABLE_DECOUPLED_DRAFT_PRELAUNCH="${DRAFT_PRELAUNCH}" \
  SGLANG_ENABLE_DECOUPLED_CHAIN_PRELAUNCH="${DRAFT_CHAIN_PRELAUNCH}" \
  SGLANG_ENABLE_DECOUPLED_DEVICE_PACK="${DRAFT_DEVICE_PACK}" \
  SGLANG_ENABLE_DECOUPLED_PER_CASE_FANOUT="${DRAFT_PER_CASE_FANOUT}" \
  SGLANG_DECOUPLED_PER_CASE_FANOUT_BUDGETS="${DRAFT_PER_CASE_FANOUT_BUDGETS}" \
  SGLANG_ENABLE_DECOUPLED_BET_PREBUILD="${DRAFT_BET_PREBUILD}" \
  CUDA_VISIBLE_DEVICES="${GPU_ID}" \
  CUDA_MPS_PIPE_DIRECTORY="${MPS_PIPE_DIR}" \
  CUDA_MPS_LOG_DIRECTORY="${MPS_LOG_DIR}" \
...
printf '%s\n' \
  ...
  "draft_chain_graph=${DRAFT_CHAIN_GRAPH}" \
  "draft_prelaunch=${DRAFT_PRELAUNCH}" \
  "draft_chain_prelaunch=${DRAFT_CHAIN_PRELAUNCH}" \
  "draft_device_pack=${DRAFT_DEVICE_PACK}" \
  "draft_per_case_fanout=${DRAFT_PER_CASE_FANOUT}" \
  "draft_per_case_fanout_budgets=${DRAFT_PER_CASE_FANOUT_BUDGETS}" \
  "draft_bet_prebuild=${DRAFT_BET_PREBUILD}" \
